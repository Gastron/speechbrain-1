    ...
    def import_model(
        ...
        **kwargs,
        ...
        if pymodule_file is None:
            ...
            module_func = os.path.join(os.path.abspath(savedir), pymodule_file)
            module_func = path_import(module_func)
            # list of tuple (name_func, func)    
            list_class = getmembers(module_func, isclass)
            if len(list_class) == 0:
                logger.INFO(
                ...
                if class_name is None:
                    print("")
                    logger.INFO(
                        "You should add class_name attribut for loading the inference class."
                    )
                    sys.exit()
                else:
                    for cls_name, cls_class in list_class:
                        if class_name == cls_name:
                            return cls_class(hparams["modules"], hparams, **kwargs)
                    sys.exit(
                        " ================ WARNING ==============="
                        "Class name" + str(cls_name) + " is not found\n"
                        "Consider renaming class name attribut to match with you class name\n\t"
                    )
